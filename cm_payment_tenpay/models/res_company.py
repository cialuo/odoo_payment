...
class ResCompany(osv.Model):
    ...
    def _get_tenpay_account(self, cr, uid, ids, name, arg, context=None):
        ...
        if tenpay_ids:
            ...
            return dict.fromkeys(ids, tenpay.tenpay_email_account)
    ...
    def _set_tenpay_account(self, cr, uid, id, name, value, arg, context=None):
        ...
        tenpay_ids = Acquirer.search(cr, uid, [
            ('website_published', '=', True),
            ('tenpay_email_account', '=', tenpay_account),
            ('company_id', '=', company_id),
        ], context=context)
        if tenpay_ids:
            Acquirer.write(cr, uid, tenpay_ids, {'tenpay_email_account': value}, context=context)
        return True
